...
class node 
{
   ...
   ~node() {delete pNext; delete pPrev;}
...
node <T> *  copy(const node <T>  * nodePtr) throw (const char *) 
{
   ...
   try
   {
    ...
    //   for ( ; nodePtr; nodePtr = nodePtr->pNext)
    //   {
    //     pDestination = insert(pDestination, nodePtr->data, true);
    //   }
      
      while (nodePtr->pNext)
      {
         pCurrent->pNext = new node <T> (nodePtr->data);
         nodePtr = nodePtr->pNext;
         pCurrent->pPrev = nodePtr->pPrev;
